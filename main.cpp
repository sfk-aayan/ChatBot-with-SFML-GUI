...
string CBot::welcome()
{
    ...
    Save_log();
    Save_log("CHATBOT");
    string ans = print_response();
    return ans;
}

void CBot::get_input(string input) //FUNCTION 28
{
...
{
    subject.erase();
    trimRight(botInput, " "); // We will remove the space added during the preprocessing of the data
...
string CBot::respond() //FUNCTION 29
{
    ...
    if(response_list.size()>0)
    {
        ...
        Save_log("CHATBOT");
        string sfml_res = print_response(); //24
        save_response();
        return sfml_res;
    }
}
...
int main()
{
    ...
    string wel = bot.welcome(); //Opening message
    //bot.Text_to_Speech();
    int flag=1;
    sf::RenderWindow window(sf::VideoMode(1130, 703), "Chatbot with SFML", sf::Style::Titlebar | sf::Style::Close | sf::Style::Resize);
    ...
    textbox1.setPosition({100, 550 });
    //string yo = "Power!";
    //text.setString(yo);
    Button btn1("Send", { 130, 80 }, 30, sf::Color::Red, sf::Color::White);
    btn1.setPosition({ 910,520 });
    btn1.setFont(font);

    sf::Texture texture;
    if (!texture.loadFromFile("bgF.png"))
    {
    ...
    sf::Text response;
    response.setString(wel);
    response.setPosition({100,260});
    response.setFont(font);
    response.setFillColor(sf::Color::Black);
    response.setCharacterSize((20));
        ...
        while (window.pollEvent(evt))
        {
            ...
            if(evt.type == sf::Event::TextEntered)
            {
                ...
                if(sf::Keyboard::isKeyPressed(sf::Keyboard::Return))
                {
                    bot.get_input(s1);
                    bot.Save_log("USER");
                    response.setString(bot.respond());
                    s1.clear();
                    textbox1.reset();
                    flag=1;
                }
                ...
                {
                    temp = evt.text.unicode;
                    if(temp[0]!= ENTER_KEY)
                        s1+=temp[temp.length()-1];
                }
            }
            ...
            if(btn1.isMouseOver(window))
            {
                if (evt.type == sf::Event::MouseButtonPressed)
                {
                    if (evt.mouseButton.button == sf::Mouse::Left)
                    {
                        ...
                        bot.get_input(s1);
                        bot.Save_log("USER");
                        string talk = bot.respond();
                        response.setString(talk);
                        s1.clear();
                        textbox1.reset();
                        flag=1;
                    }
            ...
            if(bot.botInput.find("BYE")!=string::npos)
            {
                bot.Text_to_Speech();
                flag = 0;
                bot.unknown_input_load();
                window.close();
                break;
            }
        }
